...
class DCVAE:
    def __init__(self,
        ...
        h_enc_cnn = Conv1D(cnn_units[0], kernel, activation='tanh',
                           ...
                           dilation_rate=dil_rate[0], name='cnn_0')(inputs)
        h_enc_cnn, _ = SAN(seq_len=T, pred_len=M, period_len=96, enc_in=cnn_units[0])(h_enc_cnn)  # Use norm_inputs, discard outputs

        # Middle layers
        for i in range(len(cnn_units) - 2):
            h_enc_cnn = Conv1D(cnn_units[i+1], kernel, activation='tanh',
                               strides=strs, padding="causal",
                               dilation_rate=dil_rate[i+1], name=f'cnn_{i+1}')(h_enc_cnn)
            h_enc_cnn, _ = SAN(seq_len=T, pred_len=M, period_len=96, enc_in=cnn_units[i+1])(h_enc_cnn)  # Use norm_inputs

        # Latent space
        z_mean = Conv1D(J, kernel, activation=None,
                        strides=strs, padding="causal",
                        dilation_rate=dil_rate[-1], name='z_mean')(h_enc_cnn)
        z_log_var = Conv1D(J, kernel, activation=None,
                           strides=strs, padding="causal",
                           dilation_rate=dil_rate[-1], name='z_log_var')(h_enc_cnn)
        ...
        h_dec_cnn, _ = SAN(seq_len=T, pred_len=M, period_len=96, enc_in=cnn_units[-1])(h_dec_cnn)  # Use norm_inputs

        for i in range(-2, -len(cnn_units), -1):
            h_dec_cnn = Conv1D(filters=cnn_units[i], kernel_size=kernel, activation='elu',
                               strides=strs, padding="causal",
                               dilation_rate=dil_rate[i], name=f'cnn_{i}')(h_dec_cnn)
            h_dec_cnn, _ = SAN(seq_len=T, pred_len=M, period_len=96, enc_in=cnn_units[i])(h_dec_cnn)  # Use norm_inputs
        ...
        # Loss
        MSE = -0.5 * K.mean(K.square((inputs - x__mean) / K.exp(x_log_var)), axis=-1)
        sigma_trace = -K.mean(x_log_var, axis=-1)
        log_likelihood = MSE + sigma_trace
        reconstruction_loss = K.mean(-log_likelihood)
        
        kl_loss = 1 + z_log_var - K.square(z_mean) - K.exp(z_log_var)
        kl_loss = K.mean(kl_loss, axis=-1) * -0.5
    ...
    def alpha_selection(self, load_model=False, df_X=None, df_y=None,
        ...
        if load_model:
            self.vae = keras.models.load_model(self.name+'_best_model.h5',
                                                  custom_objects={'sampling': Sampling, 'SAN': SAN},
                                                  compile = False)
        # Data
        X = df_X.values
        y = df_y.values
