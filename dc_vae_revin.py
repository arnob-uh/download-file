...
class DCVAE:
    ...
    def __init__(self,
        ...
        h_enc_cnn = RevIN(num_features=cnn_units[0], mode='norm')(h_enc_cnn)
        ...
        for i in range(len(cnn_units)-2):
            ...
            h_enc_cnn = RevIN(num_features=cnn_units[i+1], mode='norm')(h_enc_cnn)
        ...
        h_dec_cnn = RevIN(num_features=cnn_units[-1], mode='denorm')(h_dec_cnn)
        ...
        for i in range(-2, -len(cnn_units), -1):
            ...
            h_dec_cnn = RevIN(num_features=cnn_units[i], mode='denorm')(h_dec_cnn)
    ...
    def alpha_selection(self, load_model=False, df_X=None, df_y=None,
        ...
        if load_model:
            self.vae = keras.models.load_model(self.name+'_best_model.h5',
                                                  custom_objects={'sampling': Sampling, 'RevIN': RevIN},
                                                  compile = False)
        # Data
        X = df_X.values
        y = df_y.values
