...
class DCVAE:
    ...
    def __init__(self,
        ...
        h_enc_cnn = FAN(seq_len=T, enc_in=cnn_units[0], freq_topk=freq_topk, rfft=rfft)(h_enc_cnn)

        # Middle layers with FAN
        for i in range(len(cnn_units) - 2):
            ...
            h_enc_cnn = FAN(seq_len=T, enc_in=cnn_units[i+1], freq_topk=freq_topk, rfft=rfft)(h_enc_cnn)
        ...
        # Print encoder outputs before passing them to the decoder
        ...
        latent_inputs = Input(shape=(T, J), name='z_sampling')  # If J=0, this is the problem!
        ...
        h_dec_cnn = FAN(seq_len=T, enc_in=cnn_units[-1], freq_topk=freq_topk, rfft=rfft)(h_dec_cnn)
        ...
        for i in range(-2, -len(cnn_units), -1):
            ...
            h_dec_cnn = FAN(seq_len=T, enc_in=cnn_units[i], freq_topk=freq_topk, rfft=rfft)(h_dec_cnn)
    ...
    def alpha_selection(self, load_model=False, df_X=None, df_y=None,
        ...
        if load_model:
            self.vae = keras.models.load_model(self.name+'_best_model.h5',
                                                  custom_objects={'sampling': Sampling, 'FAN': FAN},
                                                  compile = False)
        # Data
        X = df_X.values
        y = df_y.values
